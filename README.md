...
| 項目 | 内容 |
|---|---|
| 天気 | その時刻の天気（快晴 / 晴れ / 曇り / 小雨 / 雨 / 雪 / 雷雨 など。WMO天気コードを日本語化、[後述](#雨の強さの表しかた)） |
| 気温 | 摂氏（℃） |
| 湿度 | 相対湿度（%） |
...
**「天気」は日中（6〜18時）の空模様から決めています。** Open-Meteo の日別 `weather_code` は24時間の最大値なので、未明に1時間だけ弱い雨があると、日中ずっと晴れの日でもその雨が「その日の天気」になってしまいます。日別の値を使うのをやめ、時間別データから次のように決めています。

| | 扱い |
|---|---|
| 雨・にわか雨・雪・にわか雪・雷雨など | **6〜21時に1時間でも出たら採用** |
| 小雨・弱い雨 | 日中に2時間以上続き、かつ**雲量50%以上・降水量0.3mm/h以上**のときだけ採用 |
| 上を満たさない弱い雨が2時間以上あるとき | 空模様に「**所により**」を付ける（例: 晴れ所により小雨） |
| 雨がないとき | 日中の平均雲量から判定（<50% 晴れ / <85% 晴れ（雲多め）/ それ以上 曇り） |

**雨や雷雨だけ21時まで見ています。** 日本の暖候期は降水の日変化のピークが20〜21時にあり、アメダスの実測で確かめたところ **6〜18時では1日の降水量の46%しか捕捉できず、雷雨の時間帯の32%が窓の外**でした。夕方の雷雨が見えないのは危険なので、重い天気だけ範囲を広げています（捕捉率62%）。空模様と弱い雨は日中のままです。

**「所により」が付くのはなぜか。** Open-Meteo の `cloud_cover` は面積率、`precipitation` は格子全体の平均です。晴れた空にごく弱い雨、という組み合わせは矛盾ではなく、**雨が格子（約5km）より小さくまばらに降る**ことをモデルが表しています。実際そういう時間帯でも、20km以内では19%で実際に雨が観測されています（完全に晴れた時間の1.6%に対して10倍以上）。「雨」と言い切るのも「晴れ」と言い切るのも実態と合わないので、含みのある言い方にしています。

**「快晴」は週間予報では使いません。** 気象庁の用語集に「予報文には用いない」と明記されているためです。時間別ブロックは1時点の状態を表すので、そちらでは出ます。

なお雲量の境界のうち **85%（曇り）は気象庁の定義（雲量9以上）に沿っていますが、50%（晴れ／晴れ（雲多め）の境目）はこの拡張が決めた線**です。気象庁の「晴れ」は雲量2〜8（20〜80%）で、この区別を持ちません。また気象庁の雲量は観測者が空全体を見て決めるもの、Open-Meteo の `cloud_cover` はモデルが格子ごとに出す面積率で、そもそも別の量です。数値を借りているだけの目安と考えてください。
...
>
> 例2（福岡 810-0001 / 2026-08-06 の実データ）: 日中13時間のうち12時間が雲量1〜13%の快晴で、18時の1時間だけ雲量96%。ただしその中身は上層雲96%・下層雲3%＝薄曇りです。日別 `weather_code` は「曇り」ですが、このブロックは「晴れ」を返します（気象庁の同日の予報も「晴れ」）。

より細かく知りたいときは、時間別ブロックで見たい時刻の天気を直接調べるのが確実です。

**「降水確率」は、その日の時間別降水確率の平均です。** Open-Meteo が返すのは「**1時間に0.1mm以上**降る確率」で、気象庁の「**6時間に1mm以上**降る確率」とは定義が違います。日別データにある24時間の最大値をそのまま使うと、24個の確率の最大を取ることになって必ず大きく膨らむため、この拡張では**時間別の値を1日ぶん平均**しています。

全国52地点×5日ぶんを気象庁の発表値と突き合わせた結果です。

| | 平均のずれ | 平均絶対誤差 | 相関 |
|---|---|---|---|
| 24時間の最大値（Open-Meteo の日別データそのまま） | +27.1 | 31.4 | 0.45 |
| **時間別の平均（この拡張）** | **−2.9** | **12.3** | **0.67** |

> 例（千葉県柏市 277-0005 / 2026-08-07 の実データ）: 日別データの最大値は 88% ですが、時間別の平均は 29% です（気象庁の同日の発表は 10%）。一方、本当に降る日は高いままです — 那覇 2026-08-07 は 98%（気象庁 90%）。

夜間も含めた24時間で平均しています。気象庁の日別の降水確率も、6時間ごと4つのブロックの最大で、そのうち2つは夜だからです。この点は「天気」とは逆になります（天気は外にいる時間の空模様を知りたいので日中だけ、降水確率は気象庁が24時間の量として定義しているので24時間）。

それでも気象庁の値とは**別の量**です。ぴったり一致はしません。

**日中（6〜18時）から決めているので、夕方以降の雨は「天気」に出ません。** 夜に雨が降る予報の日でも、日中が晴れなら「晴れ」と表示されます。夜の予報が知りたいときは時間別ブロックで該当時刻を調べてください。

**「0日後（今日）の降水量」は、すでに降った分も含むその日1日の合計です。** 「これから降る量」ではありません。降水確率も同じく1日を通した値なので、朝に見ても夜に見ても同じ数字になります。
